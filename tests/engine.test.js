...
test('traffic dragging stops at the first obstacle without jumping across it',()=>{const b=[{x:1,y:2,n:2,h:true},{x:4,y:1,n:2,h:false}];assert.deepEqual(trafficSlideLimits(b,0),{min:-1,max:1});assert.equal(slideVehicle(b,0,2),null);assert.equal(slideVehicle(b,0,3),null);assert.deepEqual(b[0],{x:1,y:2,n:2,h:true});});
test('traffic dragging respects vertical limits and allows a multi-cell slide',()=>{const b=[{x:2,y:1,n:2,h:false},{x:1,y:4,n:2,h:true}];assert.deepEqual(trafficSlideLimits(b,0),{min:-1,max:1});assert.deepEqual(trafficSlideLimits([{x:1,y:2,n:2,h:true}],0),{min:-1,max:3});assert.equal(slideVehicle([{x:1,y:2,n:2,h:true}],0,3)[0].x,4);});
