...
export class World{
 constructor(canvas){this.ctx=canvas.getContext('2d');this.assets=new SceneImages();this.ready=Promise.all(['reunion','city'].map(name=>this.assets.load(name)));this.particles=Array.from({length:24},(_,i)=>({x:(i*137.7)%540,y:(i*71.9)%960,p:i*1.3}));}
 sceneName(state){const scene=state.cinematic||(state.ending?'reunion':state.started?state.scene:'reunion');return scene==='museum'&&state.stage>=9?'museum-installed':scene;}
 sceneStatus(state){return this.assets.status(this.sceneName(state));}
 retryScene(state){return this.assets.load(this.sceneName(state),{retry:true});}
 drawChest(chest,opened){
  if(!chest)return;const c=this.ctx,sheet=this.assets.get('chest');if(!sheet)return;
 ...
 draw(state,t){
  const c=this.ctx,scene=state.cinematic|| (state.ending?'reunion':state.started?state.scene:'reunion'),img=this.assets.get(this.sceneName(state));c.clearRect(0,0,540,960);
  ...
  if(scene==='museum'&&state.stage>=10){
   const painting=this.assets.get('projection-storyboard');
